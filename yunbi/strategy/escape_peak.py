...
class Strategy(TradeClient):
    ...
    def on_minute_callback(self, minute_line):
        ...
        self.rapid_ma.put(minute_line)
        self.short_ma.put(minute_line)
        self.long_ma.put(minute_line)

        self.tick_data["ma1"].append(self.rapid_ma.get())
        ...
        if self.position < 0.9 * self.target_position:
            if 3 >= datetime.datetime.fromtimestamp(
                    minute_line.time_stamp).weekday() or 7 <= datetime.datetime.fromtimestamp(
                minute_line.time_stamp).weekday():
                return
            if self.last_timestamp:
                if minute_line.time_stamp - self.last_timestamp < 36:
                    return
            # if self.breaker_signal(minute_line) > 0:
            if cur_signal > 0 and minute_line.time_stamp - self.signal_flip_time < 997200:
