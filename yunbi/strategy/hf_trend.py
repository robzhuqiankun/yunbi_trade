...
class Strategy(TradeClient):
    ...
    class BollingSignal(object):
        ...
        def trend(self):
            ma5trd = self.minute_trend(self.tick_data["ma5"], 6)
            ma15trd = self.minute_trend(self.tick_data["ma15"], 6)
        ...
        def get_signal(self):
            ...
            if self.break_trend() > 5 or (trd > 0):
                return 1
            if self.break_trend() < -5 or (trd < 0) or (
